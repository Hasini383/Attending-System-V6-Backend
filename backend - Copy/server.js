...
const startServer = async () => {
  ...
  try {
    ...
    logSection('Database');
    const dbSpinner = startSpinner('db', 'Connecting to MongoDB...');
    await connectDB();
    ...
    const serverSpinner = startSpinner('server', 'Starting server...');
    ...
    // Keep track of connections to handle graceful shutdown
    ...
    const gracefulShutdown = (signal) => {
      ...
      // If server isn't defined or already closed, skip to database closing
      ...
      server.close(() => {
        ...
        // Clear the force shutdown timeout since we closed successfully
      ...
      function closeDBAndExit() {
        // Only attempt to close DB if mongoose is available and connected
